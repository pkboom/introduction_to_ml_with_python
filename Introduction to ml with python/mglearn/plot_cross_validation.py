...
def plot_shuffle_split():
    ...
    plt.legend(
        ...
        ["Training set", "Test set", "Not selected"],
        loc=(1, 0.3),
    )
    plt.tight_layout()
...
def plot_cross_validation():
    ...
    for i in range(n_folds):
        ...
        bars = plt.barh(
            y=range(n_folds),
            width=[n_samples_per_fold - 0.1] * n_folds,
            left=i * n_samples_per_fold,
            height=0.6,
            color=colors,
            hatch="//",
    ...
    plt.xticks(
        np.arange(n_samples_per_fold / 2.0, n_samples, n_samples_per_fold),
        ["Fold %d" % x for x in range(1, n_folds + 1)],
    )
    plt.yticks(
        np.arange(n_folds) + 0.3, ["Split %d" % x for x in range(1, n_folds + 1)]
    )
    plt.legend(
        [bars[0], bars[4]],
        ["Training data", "Test data"],
...
def plot_threefold_split():
    ...
    axis.set_xticklabels(
        ["training set", "validation set", "test set"], fontdict={"fontsize": 20}
    )
